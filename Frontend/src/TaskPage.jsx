...
const handleFile = async (e) => {
  ...
  // Let browser breathe before heavy parsing
  setTimeout(() => {
    try {
      const wb = XLSX.read(data, {
        ...
        cellDates: true,
        cellText: false,
        dense: true, // 🔥 VERY IMPORTANT for large files
      });
      ...
      // const LIMIT = 5000; // 🔥 render only first 5k rows
      // const slicedRows = rows.slice(0, LIMIT);
      setAllRows(rows);
setCurrentPage(1);
setSheetData(rows.slice(0, PAGE_SIZE));


      setWorkbook(wb);
      setSheets(wb.SheetNames);
      setSelectedSheet(firstSheet);
      setSheetData(slicedRows);

      setDivisionsBySheet({
        [firstSheet]: detectDivisions(slicedRows),
      });

      const maxCols = Math.max(
        ...slicedRows.map((r) => r.length),
        1
      );
...
const maxCols = React.useMemo(() => {
  if (!Array.isArray(sheetData)) return 0;
  return Math.max(
    ...sheetData.map(row => Array.isArray(row) ? row.length : 0),
    0
  );
}, [sheetData]);
const normalizeRow = (row, cols) => {
  const normalized = Array(cols).fill("");
...
const goToPage = (page) => {
  ...
  setCurrentPage(page);
  setSheetData(allRows.slice(start, end));
};
  ...
  >
    ⏳ Processing large Excel file… please wait
  </div>
)}
...
{allRows.length > PAGE_SIZE && (
  <div style={{ marginTop: 20, color: "#c9a646" }}>
    <button
    ...
    <span style={{ margin: "0 15px", fontWeight: 600 }}>
      ...
      {Math.min(currentPage * PAGE_SIZE, allRows.length)})
    </span>
                ...
                <tbody>
                  {filteredData.map((item, index) => {
                    ...
                    if (nonEmpty === 1) {
                      return (
                        <tr key={index}>
                          <td colSpan={maxCols + 1} style={styles.headingCell}>
