...
db.connect((err) => {
  ...
  console.log("✅ MySQL Connected (Offline)");
  ...
  db.query(createTableQuery, (err) => {
    ...
    else console.log("✅ Table 'uploaded_files' is ready!");
  });
});
...
app.get("/history/:id", (req, res) => {
  ...
  db.query(
    "SELECT file_name, json_data FROM uploaded_files WHERE id = ?",
    [fileId],
    (err, rows) => {
      if (err) {
        console.error("❌ File Load Error:", err);
      ...
      res.json({
        ...
        json_data: JSON.parse(rows[0].json_data),
      });
    }
  );
});
